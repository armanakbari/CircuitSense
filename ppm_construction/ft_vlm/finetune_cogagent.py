...
from utils.utils import llama2_text_processor, llama2_text_processor_inference, get_image_processor, compile_latex
...
def forward_step_eval(data_iterator, model, args, timers):
    def compute_metrics(eval_preds):
        ...
        for pred, label, qid in zip(decoded_preds, decoded_labels, qids):
            ...
            if args.pred_latex:
                score_comp = compile_latex('tmp', 'tmp', pred)
                score_dict['acc_compiled'].append(score_comp)
            else:
